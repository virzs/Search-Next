...
const normalizeWidgetDesktopList = (list: DesktopRootItem[]): DesktopPage[] =>
  list.filter((root) => root.id !== "dock").map((root) => ({
    ...root,
...
const persistDesktopStorage = (pages: DesktopPage[], dockItems: DesktopItem[]) => {
...
function Index() {
  ...
  const dataTypeMenuConfigMap = useMemo((): DataTypeMenuConfigMap => {
    ...
    for (const widget of widgets) {
      const schema = widget.configSnapshot?.settingsSchema || widget.settingsSchema;
  ...
  const addItemToCurrentPage = useCallback(
    (item: DesktopItem) => {
      ...
      const targetPageIndex = Math.min(currentPageIndex, sourcePages.length - 1);
  ...
  useEffect(() => {
    registerDesktopRef(desktopRef, addItemToCurrentPage, removeDesktopItemsByType);
  ...
  const handleContextMenuItemClick = useCallback(
    (item: DesktopItem, payload: ContextMenuActionPayload) => {
      ...
      setSettingsTarget({
        ...
        widgetName: item.data?.widgetConfig?.name ?? item.data?.name ?? "小组件",
