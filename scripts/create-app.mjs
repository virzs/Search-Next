...
const templatesDir = path.join(root, "scripts", "app-templates");
const appsDir = path.join(root, "apps", "apps");
...
const usage = "Usage: pnpm app:create <react|vue|solid> <app-name>";
...
const replacements = {
  __APP_NAME__: name,
  __APP_DISPLAY_NAME__: toPascalCase(name),
  __APP_CLASS_NAME__: toPascalCase(name),
  __APP_PACKAGE_NAME__: `${name}-app`,
};
...
if (await exists(targetDir)) {
  console.error(`App already exists: apps/apps/${name}`);
  process.exit(1);
}
...
console.log(`Created apps/apps/${name} from ${framework} template.`);
console.log(`Next steps:`);
console.log(`  pnpm install`);
console.log(`  pnpm --filter ${name}-app dev`);
console.log(`  pnpm --filter ${name}-app build`);
console.log(`  pnpm app:pack ${name}`);
console.log(`Template notes: scripts/app-templates/README.md`);
