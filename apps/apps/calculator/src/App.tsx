...
const readHistory = async (sdk?: AppSDK): Promise<HistoryItem[]> => {
...
const writeHistory = async (sdk: AppSDK | undefined, history: HistoryItem[]) => {
...
const formatNumber = (value: number, t: AppTranslationFn) => {
...
function IconView({ display, sizeId, press, t }: { display: string; sizeId: string; press: (key: string) => void; t: AppTranslationFn }) {
...
function FullView({ display, history, press, t }: { display: string; history: HistoryItem[]; press: (key: string) => void; t: AppTranslationFn }) {
  return (
