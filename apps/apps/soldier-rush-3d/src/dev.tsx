...
const createSdk = (sizeId: string, themeId: ThemeId) => ({
  ...
  storage: {
    get: (key: string) =>
      Promise.resolve(
        window.localStorage.getItem(`soldier-rush-3d:${sizeId}:${themeId}:${key}`) ??
          settingsDefaults[key] ??
          null,
      ),
    set: (key: string, value: string) => {
      window.localStorage.setItem(`soldier-rush-3d:${sizeId}:${themeId}:${key}`, value);
...
style.textContent = `
  :root { color-scheme: light dark; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", system-ui, sans-serif; }
	  body { margin: 0; overflow-x: hidden; background: #eef6f7; color: #1d1d1f; }
  ...
  .demo-preview { display: flex; flex-direction: column; align-items: center; gap: 7px; }
	  .demo-frame { max-width: calc(100vw - 56px); overflow: hidden; border-radius: 18px; pointer-events: auto; user-select: none; }
	  ...
	  @media (max-width: 560px) {
	    #root { padding: 14px; }
	    .demo-frame { max-width: calc(100vw - 28px); border-radius: 10px; }
	    .demo-grid { gap: 14px; }
	    .demo-section__header { align-items: start; flex-direction: column; gap: 4px; }
	  }
	`;
...
document.getElementById("root")!.innerHTML = `
  <header class="demo-hero">
    ...
    <p>自动读取 app.config.json，只展示当前启用的应用模式；此页面仅用于开发，不参与应用打包。</p>
...
for (const themeId of ["light", "dark"] as ThemeId[]) {
  ...
  const fullFrame = createPreview(grid, "full", 900, 700, themeId);
  ...
  if (config.pagePaths?.settings) {
    const settingsFrame = createPreview(grid, "settings", 520, 420, themeId);
