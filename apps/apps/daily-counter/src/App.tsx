...
const createDefaultCounters = (t: AppTranslationFn): CounterItem[] => [
...
const readCounters = async (sdk: AppSDK | undefined, defaults: CounterItem[]) => {
...
const writeCounters = async (sdk: AppSDK | undefined, counters: CounterItem[]) => {
...
function IconView({ counters, defaultCounters, sizeId, onInc, t }: { counters: CounterItem[]; defaultCounters: CounterItem[]; sizeId: string; onInc: (id: string) => void; t: AppTranslationFn }) {
...
function FullView({ counters, selectedId, setSelectedId, onInc, onDec, onAdd, onReset, t }: { counters: CounterItem[]; selectedId: string; setSelectedId: (id: string) => void; onInc: (id: string) => void; onDec: (id: string) => void; onAdd: () => void; onReset: () => void; t: AppTranslationFn }) {
  return (
...
function EditView({ counter, onChange, onAdd, t }: { counter: CounterItem; onChange: (patch: Partial<CounterItem>) => void; onAdd: () => void; t: AppTranslationFn }) {
  return (
    <div className="tw:h-full tw:overflow-auto tw:p-6">
...
function ReminderView({ counter, onInc, onDismiss, t }: { counter: CounterItem; onInc: () => void; onDismiss: () => void; t: AppTranslationFn }) {
...
function CounterPanel({ counter, active, onSelect, onInc, onDec, t }: { counter: CounterItem; active: boolean; onSelect: () => void; onInc: () => void; onDec: () => void; t: AppTranslationFn }) {
...
function CounterCard({ counter, onInc, t }: { counter: CounterItem; onInc: () => void; t: AppTranslationFn }) {
...
function CounterText({ counter, compact = false, t }: { counter: CounterItem; compact?: boolean; t: AppTranslationFn }) {
...
function ProgressRing({ counter, t }: { counter: CounterItem; t: AppTranslationFn }) {
