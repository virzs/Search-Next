...
const dateLocale = (language: AppLanguage) => language === "en-US" ? enUS : zhCN;

const monthLabel = (date: Date, language: AppLanguage) =>
  format(date, language === "en-US" ? "MMMM yyyy" : "yyyy年 M月", { locale: dateLocale(language) });
const shortMonthLabel = (date: Date, language: AppLanguage) =>
  format(date, language === "en-US" ? "MMM" : "M月", { locale: dateLocale(language) });
const weekdayLabel = (date: Date, language: AppLanguage) =>
  format(date, "EEE", { locale: dateLocale(language) });
const fullDateLabel = (date: Date, language: AppLanguage) =>
...
const weekLabels = (weekStart: WeekStart, language: AppLanguage) => {
...
const useCalendarState = ({
  ...
  language,
  t,
}: Pick<AppProps, "sdk" | "initialDate" | "events"> & {
  language: AppLanguage;
  t: AppTranslationFn;
}) => {
...
const App = ({ mode = "icon", sdk, initialDate, events }: AppProps) => {
...
const MonthGrid = ({ date, days, weekStart, language, compact = false }: { date: Date; days: CalendarDay[]; weekStart: WeekStart; language: AppLanguage; compact?: boolean }) => (
