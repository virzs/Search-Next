...
type AppLocaleInfo = { language: AppLanguage; direction: "ltr" };

const cleanups: Array<() => void> = [];
const localeListeners = new Set<(locale: AppLocaleInfo) => void>();
...
const getDevLocale = (): AppLocaleInfo => ({
  language: currentLanguage,
  direction: "ltr",
});

const setDevLanguage = (language: AppLanguage) => {
...
const createSdk = (sizeId: string, themeId: ThemeId) => ({
  appId: `clock-dev-${sizeId}-${themeId}`,
  sizeId,
  ...
  onLocaleChange: (handler: (locale: AppLocaleInfo) => void) => {
...
style.textContent = `
  ...
  body { margin: 0; background: #d8f0f2; color: #1d1d1f; }
  #root { min-height: 100vh; padding: 28px; }
  .demo-hero { margin-bottom: 24px; }
  .demo-hero h1 { margin: 0; font-size: 24px; line-height: 1.1; }
  .demo-hero p { margin: 8px 0 0; color: #5f6d72; font-size: 13px; }
  .demo-toolbar { display: flex; gap: 8px; margin-top: 14px; }
  .demo-toolbar button { border: 1px solid #c4d5d8; border-radius: 999px; background: rgba(255,255,255,0.72); color: #1d1d1f; padding: 6px 12px; font: inherit; font-size: 12px; cursor: pointer; }
  ...
  .demo-section__header p { margin: 0; color: #5f6d72; font-size: 12px; }
  .demo-grid { display: flex; flex-wrap: wrap; align-items: flex-start; gap: 20px; }
  ...
  .demo-caption { display: flex; flex-direction: column; align-items: center; gap: 2px; color: #2d3a3f; font-size: 12px; }
  .demo-caption span { color: #5f6d72; font-size: 11px; }
`;
...
document.getElementById("root")!.innerHTML = `
  <header class="demo-hero">
    ...
    <p>自动读取 app.config.json 的 sizeConfigs，只读展示所有尺寸与主题；此页面仅用于开发，不参与应用打包。</p>
