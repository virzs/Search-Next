...
export interface TablePageOptions<RecordType>
  extends Omit<Options<TableResponse<RecordType>, TabelRequestParams[]>, "defaultParams"> {
  pathname?: string;
...
export interface TablePageInstance<RecordType>
  extends Omit<Result<TableResponse<RecordType>, TabelRequestParams[]>, "data" | "params"> {
  current: number;
...
export function useTablePage<RecordType = any>(
  ...
  const { pathname, onSuccess, formatData, defaultParams, refresh: refreshOption, ...restOptions } = options ?? {};
  const location = useLocation();
  const path = pathname || location.pathname;

  const [keyword, setKeyword] = useState<string>();
  const dispatch = useAppDispatch();
  const rootTableState = useAppSelector<ITablePageState>((state) => state.tablePage, shallowEqual);
  ...
  const {
    ...
    {
      cacheKey: path,
      ...(restOptions as Options<TableResponse<RecordType>, TabelRequestParams[]>),
