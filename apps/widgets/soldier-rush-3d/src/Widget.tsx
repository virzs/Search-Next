...
import { resources, type WidgetTranslationFn, useWidgetI18n } from "./i18n";
import type { ScoreRecord, ScoreState, WidgetProps, WidgetSDK } from "./types";
...
const readScoreState = async (sdk?: WidgetSDK) => {
  try {
    if (sdk?.storage?.get) {
...
const writeScoreState = async (sdk: WidgetSDK | undefined, value: ScoreState) => {
...
const readGameSettings = async (sdk?: WidgetSDK) => {
  try {
    if (sdk?.storage?.get) {
...
const writeGameSettings = async (sdk: WidgetSDK | undefined, value: GameAudioSettings) => {
  const payload = JSON.stringify(value);
  try {
    if (sdk?.storage?.set) {
...
const Widget = ({ mode = "full", sdk }: WidgetProps) => {
