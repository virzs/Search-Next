 ...
 * @returns AI搜索响应
 */
export async function performAISearch(request: AISearchRequest): Promise<AISearchResponse> {
  ...
  try {
    // 构建AI提示词
    const systemPrompt = `你是一个专业的AI搜索助手。用户会向你提出问题，你需要：
...
请用中文回答，保持专业和友好的语调。`;
    ...
    const aiContent = response.data.choices[0]?.message?.content || "抱歉，无法获取搜索结果。";
