...
export class WallpaperService implements OnModuleInit {
  ...
  private injectRuntimeHtml(html: string) {
    const bootstrap = `<base href="./assets/"><script>(function(){var CHANNEL='search-next-wallpaper-v1';var lastActivity=0;function emit(type,payload){parent.postMessage({channel:CHANNEL,type:type,payload:payload},'*')}function activity(source){var now=Date.now();if(now-lastActivity<500)return;lastActivity=now;emit('activity',{source:source})}function apply(payload){if(!payload||typeof payload!=='object')return;var root=document.documentElement;if(payload.theme)root.dataset.theme=payload.theme;if(payload.language){root.lang=payload.language;root.dataset.language=payload.language}root.dataset.reducedMotion=payload.reducedMotion?'true':'false';root.dataset.visible=payload.visible===false?'false':'true';window.dispatchEvent(new CustomEvent('search-next-wallpaper-environment',{detail:payload}))}addEventListener('message',function(event){var data=event.data;if(data&&data.channel===CHANNEL&&data.type==='environment')apply(data.payload)});addEventListener('pointermove',function(){activity('pointer')},{capture:true,passive:true});addEventListener('pointerdown',function(){activity('pointer')},{capture:true,passive:true});addEventListener('touchstart',function(){activity('touch')},{capture:true,passive:true});addEventListener('wheel',function(){activity('wheel')},{capture:true,passive:true});addEventListener('keydown',function(event){activity('keyboard');emit('keydown',{key:event.key,code:event.code,metaKey:event.metaKey,ctrlKey:event.ctrlKey,altKey:event.altKey,shiftKey:event.shiftKey,repeat:event.repeat})},true);addEventListener('beforeunload',function(){emit('navigating',{})});function ready(){emit('ready',{})}if(document.readyState==='loading')addEventListener('DOMContentLoaded',ready,{once:true});else ready()})();</script>`;
