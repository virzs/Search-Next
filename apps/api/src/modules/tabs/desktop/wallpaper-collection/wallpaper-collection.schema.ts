...
import { WallpaperName, WallpaperType } from "../wallpaper/wallpaper.schema";

export type WallpaperCollectionDocument = WallpaperCollection & Document;

export const WallpaperCollectionName = "WallpaperCollection";

@Schema({ timestamps: true })
export class WallpaperCollection extends BaseSchema {
  ...
  @ApiProperty({ description: "绑定壁纸", type: [Object] })
  @Prop({
    type: [{ type: mongoose.Schema.Types.ObjectId, ref: WallpaperName }],
    default: [],
  })
  wallpapers: mongoose.Types.ObjectId[];
  ...
  @ApiProperty({
    description: "动态合集缓存的壁纸ID列表",
    required: false,
    type: [String],
  })
  @Prop({ type: [mongoose.Schema.Types.ObjectId], default: [] })
  cachedWallpaperIds?: mongoose.Schema.Types.ObjectId[];

  @ApiProperty({ description: "动态合集缓存刷新时间", required: false })
  @Prop({ type: Date })
  cachedAt?: Date;
}

export const WallpaperCollectionSchema =
  SchemaFactory.createForClass(WallpaperCollection);

baseSchemaMiddleware(WallpaperCollectionSchema);
