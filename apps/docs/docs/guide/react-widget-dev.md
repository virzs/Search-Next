...
- 构建产物是浏览器可直接加载的 ESM 单文件：`dist/widget-build/<name>/index.js`。
...
## 小组件示例（不打包 React）
`apps/widgets/clock/src/index.jsx` 采用 `createElement`，避免 JSX 依赖 `react/jsx-runtime`：
...
## 构建配置
`apps/widgets/clock/vite.config.js`：
...
- `inlineDynamicImports` 保证产物单文件，便于打包成 `.snwidget` 后由后端静态入口加载。

## 集成到桌面
通过 `PureWidget` 动态加载外部 ESM：

```tsx
<PureWidget config={{ entry: '/static/widgets/clock/index.js', props: { title: '时钟' }, mode: 'icon' }} />
```

或在桌面初始化时注入默认小组件（示例见 `src/pages/index/index.tsx` 中的默认注入逻辑）。

## 开发流程
1. 在 `apps/widgets/<name>` 创建目录与基础文件（参考 `apps/widgets/clock`）。
2. 组件代码导出 `mount`，使用 `globalThis.React/ReactDOM`，避免 `import react` 与 JSX。
3. 运行构建：在仓库根目录执行 `pnpm --filter <name>-widget build`（不在任务末尾自动运行）。
4. 确认生成 `dist/widget-build/<name>/index.js`。
...
## 单独启动与本地开发
小组件支持通过 pnpm workspace 独立启动开发服务器，便于脱离宿主调试：

- 安装依赖：在仓库根目录执行 `pnpm install`（已在 `devDependencies` 中声明 `react` 与 `react-dom`）。
- 启动：`pnpm --filter <name>-widget dev`（默认端口 `3002`，可在 `vite.config.js` 修改）。
- 开发页：`apps/widgets/clock/index.html` 使用本地模块导入 `react` 与 `react-dom/client`，并注入到 `globalThis` 后调用 `mount` 在两个容器中展示 `icon` 与 `full` 模式。
- 预览：`pnpm --filter <name>-widget preview` 可在本地预览构建后的静态资源。
